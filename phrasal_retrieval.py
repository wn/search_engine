def retrieve_phrase(dictionary: Dict[str, Tuple[float, Tuple[int, int], Tuple[int, int]]],
postings_file_location: str, tokens: List[str]) -> LinkedList[Tuple[str, LinkedList[int]]]:
    """
    ...
    for token in tokens[1:]:
        next_positional_index = load_positional_index(positings_file, dictionary, token)
        result = merge_positional_indexes(positional_index, next_positional_index)

    return result



def merge_positional_indexes(before: LinkedList[Tuple[str, LinkedList[int]]], after: LinkedList[Tuple[str, LinkedList[int]]]) -> LinkedList[Tuple[str, LinkedList[int]]]:
    result = LinkedList()
...
def merge_positions(before_positions: LinkedList[int], after_positions: LinkedList[int]):
    result = LinkedList()
    before_position, after_position = before_positions.get_head(), after_positions.get_head()
    ...
    while before_position is not None and after_position is not None:
        if before_position.value == after_position.value - 1:
            ...
            if before_position.skip() and before_positions.skip().value <= after_position.value - 1:
                before_position = before_positions.skip()
            else:
                before_position = before_positions.next()
        elif after_position.skip() and after.skip().value - 1 <= before_position.value:
